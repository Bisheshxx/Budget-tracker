...
8. As a user in Onboarding, I want to optionally set my display name, payday, grocery day, and Budget Target, so that I can personalise later without being forced to now.
...
12. As a user, I want the Dashboard to tell me how many days I am into the Period and how many days until payday, so that I have timing context.
...
19. As a user, I want to mark a transaction as recurring (a label), so that I can recognise my fixed commitments like rent or gym.
...
36. As a user, I want a Settings screen, so that I can update currency, Period start day, payday, grocery day, display name, and Budget Target after Onboarding.
...
- **Period** is the single budgeting unit: a monthly cycle anchored on `user_profiles.budget_period_start_day` (constrained 1–28). All targets, reports, and comparisons key off the Period. Payday is contextual only and never bounds the Period.
...
- `is_recurring` is a **label only** — no recurrence engine, no scheduler, no auto-generated future transactions in V1.
...
**Modules to build**
- Repository interfaces: `ITransactionRepository`, `ICategoryRepository`, `IReportRepository` (+ an auth abstraction so the future auth swap is contained).
...
- A recurrence engine / auto-generated recurring transactions / Recurring Expense templates — `is_recurring` is a label only.
...
- Respect the canonical vocabulary in [CONTEXT.md](../../CONTEXT.md): Period, Payday, Budget Target, Cashflow, Savings (derived), Period Comparison, Onboarding. Avoid the listed synonyms (e.g. don't call the budget unit a "month"; don't call savings a transaction).
