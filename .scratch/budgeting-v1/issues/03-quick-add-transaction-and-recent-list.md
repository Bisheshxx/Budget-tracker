...
- A one-tap quick-add form: amount, type (income/expense), category (optional for now → Uncategorized), date (defaults to today), note, and an `is_recurring` label toggle.
...
- [ ] Date defaults to today; note and `is_recurring` are captured
