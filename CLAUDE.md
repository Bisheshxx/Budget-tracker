...
- **Application code is grouped by feature** under `src/features/<feature>/` (see `docs/adr/0002`). Each feature owns its service, hook(s), schema, domain types, `components/`, and a public `index.ts` barrel that is the feature's **composition root** — it wires the service singleton to its repository (e.g. `export const authService = new AuthService(authRepository)`). **To add a feature, create `src/features/<name>/` — don't spread it across the top-level layer folders.** Domain types live in the feature as a leaf module, either `src/features/<feature>/types.ts` or a structured path such as `src/features/<feature>/types/<feature>.type.ts`; the repository **interface (port)** lives in `src/data/<feature>/IXxxRepository.ts` alongside its Supabase **implementation** and the swap-point `index.ts` (the swap point from `docs/adr/0001`). Keep each `src/data/<feature>/` folder flat until there is more than one port or implementation to organize: `I<X>Repository.ts` is the port, `supabase-<feature>-repository.ts` is the current implementation, and `index.ts` is the only public swap point imported as `#/data/<feature>`. The port imports its domain types from that leaf type module; services import the port from `#/data/<feature>/IXxxRepository` (see `docs/adr/0004`). `src/lib/` is shared infra only (Supabase client, `database.types.ts`, `money.ts`, `utils.ts`); `src/components/` is cross-feature UI only (`Header`, `Footer`, `ThemeToggle`, `ui/`). To avoid import cycles, a feature `index.ts` only wires/exports leaf modules — import context, components, and domain types from their explicit paths (`#/features/auth/contexts/auth-context`, `#/features/auth/types/auth.type`), not the barrel (the data layer imports domain types from the feature's leaf type module, so routing them through the barrel would cycle).

- **Cross-feature code lives in `src/shared/`** (see `docs/adr/0003`) — hooks, services, and domain-specific components reused by **2+ features** but owned by none. It's distinct from `src/lib/` (infra, no domain meaning) and `src/components/` (app chrome + shadcn `ui/`). Keep single-feature code in its feature; promote to `shared/` only when a **second** feature needs it. Subfolders are `components/`, `hooks/`, `services/`, `schemas/`, `types/` (add more ad hoc). `src/shared/` has **no barrel** — import leaf modules by explicit path (`#/shared/hooks/use-foo`, `#/shared/components/Foo`). See `src/shared/README.md`. Read/query primitives reused by 3+ features already live here: category and transaction domain types/schemas (`#/shared/types/category.type`, `#/shared/types/transaction.type`, `#/shared/schemas/category.schema`, `#/shared/schemas/transaction.schema`), the user profile type/hook (`#/shared/types/user-profile.type`, `#/shared/hooks/use-profile`), `use-categories`/`use-category-lookup`, `CategoryChip`/`CategoryIcon`/`CategoryPicker`, `QuickAddForm`, and the report-compute helpers (`#/shared/services/report-compute.util`) — each feature keeps only its own CRUD/service/management-UI slice.

- **Feature boundaries are ESLint-enforced** (`eslint-plugin-boundaries` in `eslint.config.js`; see `docs/adr/0009`). A feature may import only itself, `shared` (`src/shared`, `src/data`, `src/lib`, `src/components`, `src/integrations`), its own `index.ts` barrel, or — for auth specifically — `#/features/auth/contexts/*`; it may **not** reach into another feature's internals directly. Routes (`src/routes/`) may import anything. A feature's `types.ts`/`types/**` leaf module counts as that feature's public surface too (alongside its barrel), since `data/<feature>` ports already depend on it per ADR 0004. If lint flags a cross-feature import, the fix is almost always to promote the imported code to `src/shared/`, not to special-case the rule.

- **Layer cleanup constraints (see `docs/adr/0008`).** `src/routes/` is the TanStack Start application layer and should not be moved for folder-structure cleanup. shadcn primitives stay in `src/components/ui` and `components.json` should continue to point there. `src/shared/` is the global foundation and may use `components/`, `hooks/`, `services/`, `stores/`, `lib/`, `utils/`, `database/`, `types/`, `schemas/`, and `constants/`. Feature folders may mirror that shape when useful (`components/`, `hooks/`, `schemas/`, `services/`, `server/`, `types/`, `constants/`), but existing feature files can remain in place until a focused refactor moves them.
...
- **Constants:** shared constants live in `src/shared/constants/<area>.constant.ts`; feature constants live in `src/features/<feature>/constants/<feature>.constant.ts`. Use this for reusable values and domain rules such as min/max bounds, page sizes, option arrays, and date math constants. Prefer `.ts`, not `.js`, because the repo is strict TypeScript.
- **Utils:** files inside a `utils/` folder use `<area>.util.ts` naming, e.g. `period.util.ts`. Reserve `utils/` for small generic helpers; larger domain or infrastructure modules belong in `lib/`.
...
Local code-organization rules live in `docs/agents/folder-structure.md`. Use it with the global `frontend-folder-structure` skill before moving feature, shared, data, or route files.
